...
class TeacherList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        if 'popular' in self.request.GET:
            sql = "SELECT * ,COUNT(c.id) as total_course FROM main_teacher as t INNER JOIN main_course as c ON c.teacher_id=c.id GROUP BY t.id ORDER BY total_course desc"
...
class CourseList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if 'searchstring' in self.kwargs:
            ...
            if search:
                qs = models.Course.objects.filter(
                    Q(techs__icontains=search) | Q(title__contains=search))
...
def fetch_favorite_course(request, student_id, course_id):
    student = models.Student.objects.filter(id=student_id).first()
    course = models.Course.objects.filter(id=course_id).first()
...
@api_view(['DELETE'])
def delete_teacher_course(request, teacher_id, course_id):
    try:
        course = models.Course.objects.get(teacher_id=teacher_id, id=course_id)
        course.delete()
...
def teacher_change_password(request, teacher_id):
    ...
    try:
        ...
        teacherData = None
    if teacherData:
        teacherData = models.Teacher.objects.filter(
            id=teacher_id).update(password=password)
        return JsonResponse({'bool': True})
    else:
        return JsonResponse({'bool': False})
...
def student_change_password(request, student_id):
    ...
    if studentData:
        studentData = models.Student.objects.filter(
            id=student_id).update(password=password)
        return JsonResponse({'bool': True})
    else:
        return JsonResponse({'bool': False})
...
def return_certificate(request, quiz_id, student_id):
    ...
    if percentage >= 70:
        ...
        # pdf_buffer = create_certificate(data)

        # response = HttpResponse(content_type='application/pdf')
        response = StreamingHttpResponse(
            create_certificate(data), content_type='application/pdf')
...
class StudyMaterialListVideos(generics.ListAPIView):
    serializer_class = StudyMaterialSerializer
    lookup_field = 'course_id'

    def is_video_upload_file(self, uploadFile):
        file_path = uploadFile.path
    ...
    def get_queryset(self):
        ...
        queryset = models.StudyMaterial.objects.filter(course=course)
        video_uploads = [
            obj for obj in queryset if self.is_video_upload_file(obj.upload)
        ]
